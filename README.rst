This is the pyTurbSim repository.  pyTurbSim is the official water-turbulence simulation tool for hydrokinetic turbine device simulation (a.k.a. HydroTurbSim), and is also a valuable development and testing platform for NREL's 'TurbSim' tool.  New spectral models can be implemented easily in a pythonic, object-oriented manner.

This software is copyrighted by the National Renewable Energy Laboratory. It is released publically under the FreeBSD license.
...
On any system with a python installation pyTS can be executed as, e.g.:
python pyTurbSim.py TurbSim.inp
  ...or, for the GUI:
python gTurbSim.py TurbSim.inp

On must Unix-like systems the pyTurbSim.py file can be executed directly, e.g.:
./pyTurbSim.py TurbSim.inp
  ...or, for the GUI:
./gTurbSim.py TurbSim.inp

In windows, if py2exe is installed, an executable (pyTurbSim.exe) can be constructed using the command:
   python setup.py
...
3) Once the package is loaded there are multiple ways to run TurbSim.
  a) by passing the 'run' function an input file, e.g.:
        pyts.run('TurbSim.inp')           - This will write the output data to the appropriate binary files (e.g. TurbSim.bts)
  b) by passing the 'run_out' function an input file, e.g.:
        tms=pyts.run_out('TurbSim.inp')   - This DOES NOT write files, instead the TurbSim-data object is returned to 'tms'.
  c) by passing a 'tscfg' dictionary to the 'run_main' function:
        tms=pyts.run_main(*tscfg*)        - The *tscfg* object must contain all of the input parameters that are necessary for the spectral model.

##################################
# Graphical user interface (GUI) #

The pyTurbSim graphical user interface (GUI) provides a user-friendly environment for editing TurbSim input (config) files, and for running pyTurbSim (and TurbSim) using those configuration files.
...
1) cd to <turbsim root>/pyts/tslib/
...
2) You can either use:
   a) 'pyts.tsio.readBladed' or 'pyts.tsio.readAero' to return an
      array of the turbulence timeseries, or
   b) 'pyts.tsio.readModel' to read the appropriate data file, and
