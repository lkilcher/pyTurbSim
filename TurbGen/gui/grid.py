...
class deltaTextCtrl(object):
    ...
    def __init__(self, parent,
                 xCtrl, dxCtrl, nCtrl,
                 x_dflt=1.0, dx_dflt=None, n_dflt=None,
                 x_depends_on_dx=True, plus_one=True):
        """
...
class gridFigure(object):
    ...
    def __init__(self, panel, dpi):
        ...
        self.axbot.fill([0, 1, 1, 0], [0, 0, 1, 1], facecolor=[.58, .29, 0],
                        edgecolor='none', transform=self.axbot.transAxes)
...
class gridConfigFrame(ConfigFrame):
    ...
    def __init__(self, parent, id, title,
        ...
        # This function was created using wxDesigner:
        gts_wdr.GridSetup(self.panel)
        ...
        self.zCtrl = deltaTextCtrl(
            self, self.inp_height, self.inp_dz, self.inp_nz,
            x_dflt=self.config['GridHeight'], n_dflt=self.config['NumGrid_Z'])
        self.yCtrl = deltaTextCtrl(
            self, self.inp_width, self.inp_dy, self.inp_ny,
            x_dflt=self.config['GridWidth'], n_dflt=self.config['NumGrid_Y'])
        self.tCtrl = deltaTextCtrl(self,
                                   self.inp_time,
                                   self.inp_dt,
                                   self.inp_nt,
                                   x_dflt=self.config['UsableTime'],
                                   dx_dflt=self.config['TimeStep'],
                                   x_depends_on_dx=False,
                                   plus_one=0)
