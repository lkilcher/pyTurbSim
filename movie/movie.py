...
for ind in range(tsdat.shape[-1]):
    ...
    ax.set_xlabel('y [m]')
    axp.set_ylabel('z [m]')
    ax.fill(ycirc,tsdat.grid.zhub+np.sqrt(rot_rad**2-ycirc**2),'-',fc=[.8,.95,.8,1],ec='none',zorder=-10)
    ax.fill(ycirc,tsdat.grid.zhub-np.sqrt(rot_rad**2-ycirc**2),'-',fc=[.8,.95,.8,1],ec='none',zorder=-10)
    ...
    ax.quiver(y[ipo],z[ipo],dt[0,ipo]*quiv_scale,dt[2,ipo]*quiv_scale,angles='xy',scale_units='xy',scale=1,headwidth=0.01,color='b',width=0.004,headlength=1,headaxislength=1)
    ax.quiver(y[ing],z[ing],dt[1,ing]*quiv_scale,dt[2,ing]*quiv_scale,angles='xy',scale_units='xy',scale=1,headwidth=0.0001,color='r',width=.004,headlength=1,headaxislength=1)
    show()
    ax.set_ylim([1,11])
    ax.set_xlim([-5,5])
    #ax.plot([0,0],[0,20],'k--',linewidth=2)
    ax.plot([0],[1.02],'v',markersize=9,clip_on=False,transform=ax.transDataXAxesY,mec='k',mfc='k')
    ...
    axp.fill_betweenx(tsdat.z,pudat-stddev,pudat+stddev,facecolor='0.7',edgecolor='none',zorder=-8)
    ...
    axp.fill_between([0,4],tsdat.grid.zhub+rot_rad*np.ones(2),tsdat.grid.zhub-rot_rad*np.ones(2),facecolor=[.8,.95,.8,.9],edgecolor='none',zorder=-10)
...
command2=('mencoder',
          'mf://_mov2File*.png',
          '-mf',
          'type=png:w=742:h=522:fps=20', # Note that the fps (20) here matches the timestep in the movie.inp file (0.05s). This makes the movie be in realtime.
          '-ovc',
         'lavc',
          '-lavcopts',
