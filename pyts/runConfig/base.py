...
class tscfg(dict):
    ...
    def __getitem__(self,key):
        ...
        if key in ['IECstandard','IECedition'] and dict.__getitem__(self,'IECstandard').__class__ is str:
            self['IECstandard'],self['IECedition']=self.parse_IEC_standard(dict.__getitem__(self,'IECstandard'))
    ...
    def parse_IEC_standard(self,iecstd):
        ...
        if iecedt==3 and self.turbmodel=='iecvkm':
            ...
            raise InvalidConfig("If the IECstandard is 1-ed2 or 1-ed3, than the WindType must be 'NTM'.")
    ...
    def _dflt_PLExp(self,):
        ...
        if tm in ['ieckai','iecvkm']:
            if self['IECstandard']==1 and self['IEC_WindType'].lower()=='ewm':
                plexp=0.11
            elif self['IECstandard']==3:
                plexp=0.14
            plexp={'ewm':0.11,'ntm':0.14}.get(self['WindProfileType'],0.2)
        elif tm in ['wf_upw','nwtcup']:
            if self['IECturbc'].lower()=='khtest' and self.turbmodel=='nwtcup':
                plexp=0.3
            if Ri>0:
                plexp=0.14733
            else:
