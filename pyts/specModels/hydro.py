...
class tidal(specModelBase):
    ...
    Zref :      float
                Reference height at which u'w' stress nears zero.
    ...
    def __call__(self,tsrun):
        ...
        out[:]=(out.sigma2[:,:,None,None]*self.coef[:,0][:,None,None,None]/dudz)/(1+self.coef[:,1][:,None,None,None]*(tsrun.grid.f[None,None,None,:]/dudz)**self.pow5_3)
        return out
...
class river(tidal):
    ...
    coef=np.array([[1.057,3.432],[0.351,0.546],[0.265,0.341]],dtype=ts_float)
    # These fit negative velocity data in eastriver:
    #coef=np.array([[0.784,2.085],[0.272,0.357],[0.240,0.290]])
