...
class NWTCgenModel(turbModelBase):
    ...
    @property
    def zL(self,):
        return zL(self.Ri,self.turbmodel)
        ...
        for iz in range(out.n_z):
            for iy in range(out.n_y):
                ...
                for comp in out.grid.comp:
                    self._work[:]=0.0
                    if self.stable:
                        out[comp][iz,iy]=self._stableModel(z,u,comp,self.coefs[comp])
                    else:
                        out[comp][iz,iy]=self._unstableModel(z,u,comp,self.coefs[comp])
        return out
...
class smooth(NWTCgenModel):
    ...
    def initCoefs(self,):
        ...
        if self.stable:
            ...
            self.coefs=np.ones((3,2,2),dtype=ts_float)
...
class nwtcup(NWTCgenModel):
    ...
    def initCoefs(self,):
        if self.stable:
            loc_zL=fix2range(self.zL,0.005,3.5)
            ustar_tmp=0.
        else:
            loc_zL=np.abs(fix2range(self.zL,-0.5,-0.025))
        ...
        for i0,sfx in enumerate(['il','ih']):
            for i1,pfx in enumerate(['Pr','fr']):
                nm=pfx+'_'+sfx
                arr=dat['terms'][nm]
                out[:,i0,i1]=fix2range(arr[:,0]*(loc_zL**arr[:,1])*ustar_tmp**arr[:,2]*np.exp(arr[:,3]*loc_zL+arr[:,4]*ustar_tmp),dat['min'][nm],dat['max'][nm])
