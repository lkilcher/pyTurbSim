...
class spec_fig(figobj):
    
    def __init__(self,specobj,fignum=1201,axsize=3,frame=[.6,.3,1,.3],gap=.4,xscale='log',yscale='log',**kwargs):
        if specobj.__class__ is not list:
            specobj=[specobj]
        nax=(len(specobj[0]),len(specobj))
        self.specobj=specobj
        super(spec_fig,self).__init__(fignum,nax,axsize=axsize,frame=frame,gap=gap,**kwargs)
        self.axes=np.empty((nax[1]),dtype='O')
        for idx,axs in enumerate(self.sax.ax.T):
            self.ax[idx]=axgroup(axs)
        self.scale=(xscale,yscale)

    def __iter__(self,):
        for sp,axgrp in zip(self.specobj,self.ax):
            yield sp,axgrp

    def plot_specobj(self,specobj,axgroups=None,**kwargs):
        if axgroups is None:
            axgroups=self.ax
        for axg in axgroups:
            for idx,ax in enumerate(axg):
                try:
                    ax.plot(specobj.freq,specobj[idx].T,**kwargs)
                except:
                    ax.plot(specobj.freq,specobj[idx],**kwargs)
            
    def draw(self,*args,**kwargs):
        for sp,ax in self:
            ax.iterplot(sp.iter4axgroup(),*args,**kwargs)

    def label(self,col=0,pos='ur',offset=10,**kwargs):
        for ax,sp in zip(self.ax[col],self.specobj[col]):
            ax.annoteCorner(sp.name,pos=pos,offset=offset,**kwargs)

    def hide_xticklabels(self,):
        self.sax.hide('xticklabels')
    def hide_yticklabels(self,):
        self.sax.hide('yticklabels')

    def xlabel(self,label=None,**kwargs):
        if label is None and hasattr(self.specobj[0].freq,'label'):
            label=self.specobj[0].freq.label
        self.sax[-1,0].set_xlabel(label,**kwargs)
    def ylabel(self,label=None,rotation=0,**kwargs):
        if label is None:
            label=self.specobj[0][0].label
        self.sax[-1,0].set_ylabel(label,rotation=rotation,**kwargs)

    def __exit__(self,*args):
        for ax in self.ax:
            ax.set_xscale(self.scale[0])
            ax.set_yscale(self.scale[1])
        self.hide_xticklabels()
        self.xlabel()
    ...
    def set_xlim(self,*args,**kwargs):
        for axg in self.ax:
            axg.set_xlim(*args,**kwargs)

    def set_ylim(self,*args,**kwargs):
        for axg in self.ax:
            axg.set_ylim(*args,**kwargs)

class cohere_fig(spec_fig):
    def __init__(self,specobj,fignum=2201,axsize=3,frame=[.6,.3,1,.3],gap=.4,xscale='log',yscale='linear',**kwargs):
        if specobj.__class__ is not list:
            specobj=[specobj]
        nax=(len(specobj[0]),len(specobj))
        self.specobj=specobj
